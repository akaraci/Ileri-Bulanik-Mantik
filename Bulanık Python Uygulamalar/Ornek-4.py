...
ax0.set_xticks(np.arange(min(var_sicaklik),81,5))
...
#-----Larsen
# rule1 = (np.fmin(sicaklik_fit_low, sd_fit_low)*set_motorhizi_fast)
# rule2 =(np.fmin(sicaklik_fit_med, sd_fit_med)*set_motorhizi_slow)
# rule3 = (np.fmin(sicaklik_fit_low, sd_fit_med)*set_motorhizi_fast)
# rule4 = (np.fmin(sicaklik_fit_med, sd_fit_low)*set_motorhizi_med)
...
#out_set_final=np.fmax(np.fmax(np.fmax(rule1,rule2),rule3),rule4)
